...
import { useMyShowcases, useDeleteShowcase } from "@/hooks/query/showcase";
...
export default function ProfileLayoutPage() {
  ...
  const { data: showcases = [], isLoading: showcaseLoading } = useMyShowcases();
  ...
  const handleDeleteShowcase = (id: string) => {
    showAlert("삭제 확인", "이 아바타 자랑을 삭제하시겠습니까?", "삭제", () => {
      ...
      <Card
        ...
        headerAction={
          <div
            ...
            className="flex cursor-pointer items-center justify-between rounded-xl text-[11px] font-bold text-slate-300 transition-all hover:bg-white/5 focus:bg-white/5 focus:text-white"
          >
